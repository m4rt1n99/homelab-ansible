...
function dev_block() {
	# map partitions (e.g. sdc1, nvme0n1p1) to their parent disk
	if [ -e "/sys/class/block/$1/partition" ]; then
		basename "$(readlink -f "/sys/class/block/$1/..")"
	else
		echo "$1"
	fi
}
...
function check_dev() {
	...
	# normalize optional '/dev/' prefix from config
	DEV="${DEV#/dev/}"

	# resolve symlinked /dev paths (e.g. /dev/disk/by-id, /dev/disk/by-partuuid)
	if [ -L "/dev/$DEV" ]; then
		DEV_REAL="$(basename "$(readlink -f "/dev/$DEV")")"
		if [ -n "$DEV_REAL" ]; then
			log "recognized disk: ${DEVICES[$1]} --> $DEV_REAL"
			DEV="$DEV_REAL"
			DEVICES[$1]="$DEV_REAL"
		fi
	fi
	...
	DEV_BLOCK="$(dev_block "$DEV")"
	if [ "$DEV_BLOCK" != "$DEV" ]; then
		log "using parent block device: $DEV --> $DEV_BLOCK"
		DEV="$DEV_BLOCK"
		DEVICES[$1]="$DEV_BLOCK"
	fi
